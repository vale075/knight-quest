...
func end():
	...
	$cinema/blacktext/text.text="TON AIDE EST \n TROP LÉGERE POUR \n ETRE REMARQUÉ"
	...
	file.close()
	if $mur/base.pv<5:
		$cinema/blacktext/text.text="JE NE TE \n REMERCIE DONC PAS"
		$cinema/blacktext.visible=true
		yield(self,"ok")
		$cinema/blacktext/text.text=""
		$cinema/blacktext.visible=false
		...
		$cinema/blacktext/text.text="TU NOUS CONDUIRAS \n A LA MORT"
		$cinema/blacktext.visible=true
		yield(self,"ok")
		$cinema/blacktext/text.text=""
		$cinema/blacktext.visible=false
		...
		for i in save:
			save_game.store_line(i)
		save_game.close()
		get_tree().change_scene("res://level10good.tscn")
