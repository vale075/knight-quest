...
func end():
	...
	$cinema/blacktext/text.text="JE SAVAIS QU'ON \n POUVAIS TE FAIRE \n CONFIANCE! TU \n NOUS SAUVE!!"
	...
	file.close()
	if $mur/base.pv<5:
		$cinema/blacktext/text.text="TU N'EST PAS \n COMME LES AUTRES"
		$cinema/blacktext.visible=true
		yield(self,"ok")
		$cinema/blacktext/text.text=""
		$cinema/blacktext.visible=false
		...
		if not lignee=="":
			var save={
				"res://level10veryveryveryverygood.tscn"  :  "",
		...
		save_game.close()
		get_tree().change_scene("res://level10veryveryveryverygood.tscn")
	else:
		$cinema/blacktext/text.text="MAIS JE GARDE \n CETTE HAINE ENVERS \n VOUS, LES HUMAINS"
		$cinema/blacktext.visible=true
		yield(self,"ok")
		$cinema/blacktext/text.text=""
		$cinema/blacktext.visible=false
		...
		if not lignee=="":
			var save={
				"res://level10veryveryverygood.tscn"  :  "",
				pvextract+str($mur/base.pv)  :  "",
				lignee : ""
			}
		else:
		...
		for i in save:
			save_game.store_line(i)
		save_game.close()
		get_tree().change_scene("res://level10veryveryverygood.tscn")
