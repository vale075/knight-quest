...
func end():
	...
	$cinema/blacktext/text.text="TU EST \n UN DÉMON!"
	...
	file.close()
	if $mur/base.pv<5:
		$cinema/blacktext/text.text="AU SERVICE DU \n DIABLE..."
		$cinema/blacktext.visible=true
		yield(self,"ok")
		$cinema/blacktext/text.text=""
		$cinema/blacktext.visible=false
		...
		if not lignee=="":
			var save={
				"res://level10veryveryverybad.tscn"  :  "",
		...
		save_game.close()
		get_tree().change_scene("res://level10veryveryverybad.tscn")
	else:
		$cinema/blacktext/text.text="COMMENT QUELQU'UN\n COMME TOI PEUT \nEXISTER?"
		$cinema/blacktext.visible=true
		yield(self,"ok")
		$cinema/blacktext/text.text=""
		$cinema/blacktext.visible=false
		...
		if not lignee=="":
			var save={
				"res://level10veryveryveryverybad.tscn"  :  "",
				pvextract+str($mur/base.pv)  :  "",
				lignee : ""
			}
		else:
		...
		for i in save:
			save_game.store_line(i)
		save_game.close()
		get_tree().change_scene("res://level10veryveryveryverybad.tscn")
