...
func end():
	...
	if $mur/base.pv<5:
		...
		if not lignee=="":
			var save={
				"res://level8veryveryverygood"  :  "",
				pvextract+str($mur/base.pv)  :  "",
				lignee : ""
			}
		else:
			var save={
				"res://level8veryveryverygood.tscn"  :  "",
				pvextract+str($mur/base.pv)  :  ""
			}
		for i in save:
			save_game.store_line(i)
		save_game.close()
		get_tree().change_scene("res://level8veryveryverygood.tscn")
	else:
		var save_game = File.new()
		save_game.open("res://save/save.tres", File.WRITE)
		if not lignee=="":
			var save={
				"res://level8veryverygood.tscn"  :  "",
				pvextract+str($mur/base.pv)  :  "",
				lignee : ""
			}
		else:
		...
		for i in save:
			save_game.store_line(i)
		save_game.close()
		get_tree().change_scene("res://level8veryverygood.tscn")
