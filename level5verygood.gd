...
func end():
	...
	$sond/musique.play()
	$cinema/blacktext/text.text="DESOLER... \n ESCUSE MON \n ROI..."
	...
	$cinema/blacktext/text.text="IL S'EST EMPORTER \n ... IL NE TE \n VEUT AUCUN MAL..."
	$cinema/blacktext.visible=true
	yield(self,"ok")
	$cinema/blacktext/text.text=""
	$cinema/blacktext.visible=false
	...
	if $mur/base.pv<5:
		...
		save_game.close()
		get_tree().change_scene("res://level6veryverygood.tscn")
	else:
		var save_game = File.new()
		save_game.open("res://save/save.tres", File.WRITE)
		var save={
			"res://level6verygood.tscn"  :  "",
			pvextract+str($mur/base.pv)  :  ""
			}
		for i in save:
			save_game.store_line(i)
		save_game.close()
		get_tree().change_scene("res://level6verygood.tscn")
func touch():
	pass