...
func end():
	...
	file.close()
	if $mur/base.pv<5:
		$cinema/blacktext/text.text="SOIT PLUS \n RADICAL! TUE \n PLUS!"
		$cinema/blacktext.visible=true
		yield(self,"ok")
		$cinema/blacktext/text.text=""
		$cinema/blacktext.visible=false
		...
		save_game.close()
		get_tree().change_scene("res://level7bad.tscn")
	else:
		$cinema/blacktext/text.text="NOUS SOMME EN \n TRAIN DE LES \n ANÉANTIR! CONTINUE!!!"
		$cinema/blacktext.visible=true
		yield(self,"ok")
		$cinema/blacktext/text.text=""
		$cinema/blacktext.visible=false
		...
		for i in save:
			save_game.store_line(i)
		save_game.close()
		get_tree().change_scene("res://level7verybad.tscn")
